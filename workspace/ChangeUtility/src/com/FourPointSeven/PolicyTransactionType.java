...
public class PolicyTransactionType extends FourPointSevenBase {
	...
	public void TestCase(String username, String password, String mfaUsername, String mfaPassword, String applicationId, String transactionType, String status ) throws InterruptedException{
		...
		try {
			...
			WebElement policiesDiv=getElementByXPath("HealthcarepoliciesDiv");
			List<WebElement> tables;
		try{
			 tables=policiesDiv.findElements(By.tagName("table"));
		}
		catch(NullPointerException e)
		{
			 tables=policiesDiv.findElements(By.tagName("table"));
		}
			System.out.println("number of tables in polices tab is:"+tables.size());
			List<WebElement> policiesTabledata0=tables.get(0).findElements(By.tagName("td"));
			List<WebElement> policiesTableheaders=tables.get(0).findElements(By.tagName("th"));
			boolean header=false;
			for(int i=0;i<policiesTableheaders.size();i++)
				{
					WebElement headers=policiesTableheaders.get(i);
					if(headers.getText().contains(transactionType))
					{
						header=true;
						highLightElement(headers);
						screenshot.pageScreenshot();
						unhighlightElement(headers);
						Testfail=false;
						WebElement data=policiesTabledata0.get(i);
						...
						if(Result.trim().contains(status.trim()))
						{
							//System.out.println("entered the if loop" +status);
							Testfail=false;
